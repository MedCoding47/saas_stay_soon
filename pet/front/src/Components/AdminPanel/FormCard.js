...
const FormCard = (props) => {
  ...
  const handleApprove = async () => {
    setIsApproving(true);
    try {
      const response = await api.put(`/adoptions/${props.form.id}/approve`, {
  ...
  const deleteFormAdoptedPet = async () => {
    try {
      const deleteResponse = await api.delete(`/adoptions/${props.form.petId}`);
  ...
  const handleReject = async () => {
    setIsDeleting(true);
    try {
      const response = await api.delete(`/adoptions/${props.form.id}`);
