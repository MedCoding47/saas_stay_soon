...
export default function VeterinaireDashboard() {
  ...
  const AvatarCard = () => (
    <div className="relative h-full w-full bg-[#0D0D0D] rounded-3xl p-8 overflow-hidden">
      <div className="h-full flex flex-col items-center justify-center text-center">
        ...
        <h2 className="font-display font-black text-white text-3xl mt-4">{profile.userName || 'Veterinaire'}</h2>
    ...
    <Card className="h-full bg-white rounded-3xl border border-[#E8E0D8] p-6">
      ...
      <div>
        {profile.formation ? (
          ...
          <p className="text-sm text-[#8c7e74]">No credentials added yet.</p>
        )}
      </div>
    </Card>
  );
  ...
  const DescriptionCard = () => (
    <Card className="relative h-full w-full overflow-hidden bg-white rounded-3xl border border-[#E8E0D8] p-6">
      {profile.description ? (
        <>
          <p className="text-xs font-bold tracking-widest uppercase text-[#8c7e74] mb-4">About</p>
          <p className="text-sm text-[#0D0D0D] leading-relaxed">{profile.description}</p>
        </>
      ) : (
        <div className="h-full flex items-center justify-center">
          <p className="text-sm text-[#8c7e74] text-center">No description yet. Edit your profile to add one.</p>
        </div>
      )}
    </Card>
  );
  ...
  const StatsCard = () => (
    <Card className="flex h-full flex-col justify-between bg-white rounded-3xl border border-[#E8E0D8] p-6">
      ...
      <div className="mt-4">
        ...
        <p className="text-sm text-[#8c7e74] mt-1">Advice articles shared</p>
      </div>
    ...
    <Card className="relative h-full w-full overflow-hidden bg-white rounded-3xl border border-[#E8E0D8] p-6">
      ...
      <div>
        {bookings.length > 0 ? (
          ...
          <p className="text-sm text-[#8c7e74]">No recent activity.</p>
        )}
      </div>
    </Card>
  );
    ...
    <PageTransition>
      ...
      <div className="bg-white border-b border-[#E8E0D8] px-8 py-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <h1 className="font-display font-black text-3xl text-[#0D0D0D]">Veterinaire Dashboard</h1>
      ...
      <main className="min-h-screen bg-[#FAF7F2] pb-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
                ...
                <div className="max-w-2xl mx-auto">
                  <Card className="bg-white rounded-3xl border border-[#E8E0D8] p-6">
                    <CardHeader className="p-0 mb-6">
                      <CardTitle className="text-xs font-bold tracking-widest uppercase text-[#8c7e74]">Edit Clinic Profile</CardTitle>
                    </CardHeader>
                    <CardContent className="p-0">
                      <div className="space-y-4">
                        <div>
                          <label className="block text-sm font-semibold text-[#0D0D0D] mb-1">Clinic Name</label>
                        ...
                        <div>
                          <label className="block text-sm font-semibold text-[#0D0D0D] mb-1">Location</label>
                          <input name="location" value={editForm.location} onChange={handleEditChange}
                            className="w-full rounded-2xl border-2 border-gray-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-coral/40 focus:border-coral transition-all" />
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-[#0D0D0D] mb-1">Phone</label>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <label htmlFor="isAvailable" className="text-sm font-semibold text-[#0D0D0D]">Available for consultations</label>
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-[#0D0D0D] mb-1">Formation / Credentials</label>
                        ...
                        <div>
                          <label className="block text-sm font-semibold text-[#0D0D0D] mb-1">Description</label>
          ...
          {tab === 'advice' && (
            <div>
              <div className="flex items-center justify-between mb-6">
                <p className="text-sm text-[#8c7e74]">Share tips and advice for pet owners</p>
                <Button variant="primary" className="!rounded-full" onClick={() => setAdviceModal(true)}>Add Advice</Button>
              </div>
              {advice.length === 0 ? (
                <div className="text-center py-20 text-[#8c7e74]">No advice yet.</div>
              ) : (
              ...
              <Modal isOpen={adviceModal} onClose={() => setAdviceModal(false)} title="Add Advice">
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-semibold text-[#0D0D0D] mb-1">Title</label>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-[#0D0D0D] mb-1">Content</label>
          ...
          {tab === 'recommendations' && (
            <div>
              <div className="flex items-center justify-between mb-6">
                <p className="text-sm text-[#8c7e74]">Product care tips for pet owners</p>
                <Button variant="primary" className="!rounded-full" onClick={() => setRecoModal(true)}>Add Tip</Button>
              </div>
              {recommendations.length === 0 ? (
                <div className="text-center py-20 text-[#8c7e74]">No recommendations yet.</div>
              ) : (
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <Modal isOpen={recoModal} onClose={() => setRecoModal(false)} title="Add Recommendation">
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-[#0D0D0D] mb-1">Target Species (optional)</label>
                    <input value={recoForm.targetSpecies} onChange={(e) => setRecoForm({ ...recoForm, targetSpecies: e.target.value })}
                      placeholder="e.g. Dog, Cat, Rabbit"
                      className="w-full rounded-2xl border-2 border-gray-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-coral/40 focus:border-coral" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-[#0D0D0D] mb-1">Target Age Range (optional)</label>
          ...
          {tab === 'bookings' && (
            <div>
              {bookings.length === 0 ? (
                <div className="text-center py-20 text-[#8c7e74]">No booking requests yet.</div>
              ) : (
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-[#8c7e74]">{new Date(b.bookingDate).toLocaleDateString()} — {b.notes || 'No notes'}</p>
                          </div>
                        </div>
                        {b.petName && <p className="text-xs text-[#8c7e74] mt-2 ml-13">Pet: {b.petName}</p>}
                      </div>
