...
export default function AdminDashboard() {
  const [stats, setStats] = useState({ totalPets: 0, available: 0, pending: 0, adopted: 0 });
    ...
    Promise.all([
      api.get('/pets').then(({ data }) => {
        ...
        setStats({
          ...
          pending: pets.filter((p) => p.status === 2 || p.status === 3 || p.status === 4 || p.status === 'ApplicationReceived' || p.status === 'UnderReview' || p.status === 'Approved').length,
  ...
  const statCards = [
    { label: 'Total Pets', value: stats.totalPets, color: 'bg-coral/10 text-coral', icon: '🐾' },
    { label: 'Available', value: stats.available, color: 'bg-teal/10 text-teal', icon: '✅' },
    { label: 'Pending', value: stats.pending, color: 'bg-amber-100 text-amber-700', icon: '⏳' },
    { label: 'Adopted', value: stats.adopted, color: 'bg-blue-100 text-blue-700', icon: '🏠' },
  ];
      ...
      <div className="min-h-screen bg-warm-light">
        ...
        <div className="pl-64 pt-16">
          <div className="max-w-7xl mx-auto px-8 py-8">
            <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-2xl font-bold text-gray-900 mb-8">Dashboard</motion.h1>
            ...
            <div className="grid lg:grid-cols-2 gap-8">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card">
                ...
                <table className="w-full text-sm">
                  <thead><tr className="text-gray-500 border-b border-gray-100"><th className="text-left pb-3 font-medium">Name</th><th className="text-left pb-3 font-medium">Breed</th><th className="text-right pb-3 font-medium">Status</th></tr></thead>
                  <tbody>
                    {recentPets.map((pet) => (
                  ...
                  <ul className="space-y-3">
                    {adoptions.slice(0, 6).map((a) => (
                      <li key={a.id} className="flex items-center justify-between py-2 border-b border-gray-50">
                        <span className="text-sm text-gray-700">Request <span className="font-medium">#{a.id}</span></span>
